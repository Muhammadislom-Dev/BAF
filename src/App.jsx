  ...
  return (
    <div className="App">
      <Routes>
        ...
        {
          category.map((e) => (
            <Route  path={`/productpage${e.id}`} element={<ProductPage name={e.name_en} img={e.image}  />} />
          ))
        }
        ...
        {
          data.map((item) =>(
            <Route path={`/productaboutpage${item.id}`}   element={<ProductAboutPage name={item.name_en} img1={item.image1} cost={item.cost} img2={item.image2} img3={item.image3} description={item.description_en} />} />
          ))
        }
      </Routes>
    </div>
